...
# def gradient_formula(b):
#     x_not = sy.symbols('x')
#     y_not = sy.symbols('y')
#     return sy.diff(f(x_not,y_not,b), x_not), sy.diff(f(x_not,y_not,b), y_not)
#
# def gradient(x, y, b):
#     x_not = sy.symbols('x')
#     y_not = sy.symbols('y')
#     dx, dy = gradient_formula(b)
#     return dx.subs(x_not, x), dy.subs(y_not, y)
#
# def get_next_x_and_y(xk, yk, b, s): #(xk+1,yk+1)=(xk,yk)−s∇f(xk,yk)
#     dx, dy = gradient(xk,yk,b)
#     return xk-(s*dx), yk-(s*dy)
